            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div v-show="activeTab === 'geral'" class="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6">
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                        <label class="block text-[10px] font-black uppercase text-gray-400 mb-4 tracking-widest">Fotos (Arraste para ordenar)</label>
                        ...
                        <div class="md:col-span-2 space-y-6">
                            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                                ...
                                <div class="mt-8 p-6 bg-green-50 rounded-2xl border border-green-100 flex justify-between items-center">
                                    <div><p class="text-[10px] font-black text-green-700 uppercase mb-1">Lucro</p><p class="text-3xl font-black text-green-600 tracking-tighter">{{ profitData.value }}</p></div>
                                    <div class="text-right"><p class="text-[10px] font-black text-green-700 uppercase mb-1">Margem</p><p class="text-3xl font-black text-green-600 tracking-tighter">{{ profitData.percentage }}%</p></div>
                                </div>
                            </div>
                            ...
                            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                                <div class="flex items-center justify-between mb-6">
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group">
                                        <span class="text-[10px] font-black uppercase text-gray-400 group-hover:text-indigo-600 transition">Frete Grátis</span>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <input v-model="form.weight" type="number" step="0.001" placeholder="Peso (kg)" class="border-gray-100 bg-gray-50 rounded-xl font-bold text-sm" />
                                    <input v-model="form.width" type="number" placeholder="Larg (cm)" class="border-gray-100 bg-gray-50 rounded-xl font-bold text-sm" />
                                    <input v-model="form.height" type="number" placeholder="Alt (cm)" class="border-gray-100 bg-gray-50 rounded-xl font-bold text-sm" />
                                    <input v-model="form.length" type="number" placeholder="Comp (cm)" class="border-gray-100 bg-gray-50 rounded-xl font-bold text-sm" />
                                </div>
                            </div>
                        </div>

                        <div class="bg-indigo-900 p-8 rounded-3xl shadow-xl text-white relative h-fit">
                            <h3 class="flex items-center text-xs font-black uppercase opacity-60 mb-6 italic">Promoção</h3>
                            <div class="space-y-4">
                                <input v-model="form.promo_price" type="number" step="0.01" class="w-full bg-indigo-800 border-none rounded-2xl font-black text-white" placeholder="Preço Promo (R$)" />
                                <div><label class="text-[9px] font-black uppercase opacity-40">Início</label><input v-model="form.promo_start_at" type="datetime-local" class="w-full bg-indigo-800 border-none rounded-xl text-[10px] text-white" /></div>
                                <div><label class="text-[9px] font-black uppercase opacity-40">Fim</label><input v-model="form.promo_end_at" type="datetime-local" class="w-full bg-indigo-800 border-none rounded-xl text-[10px] text-white" /></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-show="activeTab === 'seo'" class="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6">
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        <div class="flex items-center gap-3"><Code class="w-5 h-5 text-indigo-600" /><h3 class="text-xs font-black uppercase tracking-widest text-gray-500">Rastreamento</h3></div>
                        <textarea v-model="form.google_tag_manager" rows="3" class="w-full border-gray-100 bg-gray-50 rounded-2xl text-[11px] font-mono" placeholder="GTM Script..."></textarea>
                        <div class="grid grid-cols-2 gap-4">
                            <input v-model="form.ads" type="text" placeholder="ID Google Ads" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" />
                            <input v-model="form.canonical_url" type="text" placeholder="URL Canônica" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" />
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        ...
                        <input v-model="form.meta_title" type="text" placeholder="Meta Title" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" />
                        ...
                        <div class="flex flex-wrap gap-2 p-3 border border-gray-100 bg-gray-50 rounded-2xl min-h-[50px]">
                            ...
                            <input v-model="tagInput" @keydown.enter.prevent="addTag" type="text" class="flex-1 bg-transparent border-none focus:ring-0 text-sm font-bold p-0" placeholder="TAG + ENTER" />
                        </div>
                        
                        <textarea v-model="form.meta_description" rows="2" placeholder="Meta Description" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold"></textarea>
                    </div>

                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        <div class="flex items-center gap-3"><FileText class="w-5 h-5 text-green-600" /><h3 class="text-xs font-black uppercase tracking-widest text-gray-500">Conteúdo</h3></div>
                        <div class="grid grid-cols-2 gap-4">
                            <input v-model="form.h1" type="text" placeholder="H1 principal" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" />
                            <input v-model="form.h2" type="text" placeholder="H2 secundário" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" />
                        </div>
                        <textarea v-model="form.text1" rows="3" placeholder="Texto de apresentação..." class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold"></textarea>
                        <textarea v-model="form.schema_markup" rows="2" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-mono text-[10px]" placeholder='Schema JSON-LD...'></textarea>
                    </div>
                </div>
